...
{		
   ...
   //alert("initPageCalendar");
	...
	//GET CALENDAR EVENTS FROM CLOUDANT
	 $.ajax({
            //url: "http://ssodemyapp.mybluemix.net/schede/addScheda",
			url: rooturl+link,
            type: "GET"})
        .done(function(data) {
			...
			if (role.toLowerCase()!="muzio")
			{
				
				getEventsFeed({
					url: 'http://gse.dst.ibm.com/sales/gss/download/repenabler/FeedProvider?repid=giovanna_massari@it.ibm.com&pagetype=rep&country=IT&language=it&item=events&type=rss&callback=?',
					evtype: "event_feed_pp"
				},function() {
					
					
					getEventsFeed({
						url: 'http://www.ibm.com/events/it/it/index.rss?ca=rss_eventi&me=W&met=eventirh',
						evtype:"event_feed_ibm"
					},function() {
						sortEvents();
...
function getEventsFeed(options,callback){
		...
		colog("getEventsFeed from url "+opt.url)
		...
		if  ((role=="customer") && (evtype="events_feed_pp")){
			
			var $c=filterRows($customers,{
				customername: company
			})
			
			colog("$c--->"+JSON.stringify($c));
			var pp=$c.rows[0].doc.isr_personalpage;
			var email=$c.rows[0].doc.isr_email;
			repurl=repurl_templ.replace("{repemail}",email);
			colog("customer role and events_feed_pp rss, switching feeds on url "+repurl)
            ...
            .done(function(data) {
				colog("rss feed: "+JSON.stringify(data));
...
$.ajaxPrefilter(function(options, originalOptions, jqXHR) {
    ...
    // create the data and add the sessionId
	 ...
	 //load customers
      ...
      $.ajax({
            url: rooturl+"/events/listobjects/customers",
            type: "GET"})
         .done(function(data) {
			 colog("loaded customers: "+JSON.stringify(data));
			 //callback(data);
			 $customers=data;
		 }); 	 
 ...
 function filterRows($m,filt) {
	 ...
	 function socialLogin(tipo) {
		 ...
		 colog("socialLogin")
		  ...
		  var htmMessage= new EJS({url: tplurl+'tpl/sociallogin.ejs'}).render($socialuser); 		  
		  $("#page_sociallogin #socials").html(htmMessage);
		  $.mobile.changePage("#page_sociallogin");
				  
		 }else
		 {
		  var url=rooturl+"/"+tipo+"/auth";	
	 ...
	 function gochat() {
	nickname=$("#page_chatlogin #nickname").val();
	setCookie("nickname",nickname)
	
	chatSend("connect")
	
	$.mobile.changePage("#page_chat");
	$("#page_chat #status").html("");
	$("#page_chat #msg").val("");
	
	
}

var room="demy*dinoravasi";

function chatSend(obj){
	
	if (obj=="typing"){
		socket.emit('typing', { message: "", user: nickname });
		
		return;
	}
	
	if (obj=="connect"){
		socket.emit('connected', { message: "", user: nickname, room: room });
	...
	if ($.trim(text)=="") return;
	colog("sending "+text);
	socket.emit('message', { message: text, user: nickname, room: room });
	i.value="";
...
function initChat(){
		...
		socket.on('connect', function() {
		   	 colog("connected to the socket server");
        ...
        socket.on('message', function(msg) {
               colog("received message from sockserver: "+JSON.stringify(msg));
			   var bubbleclass="bubble bubble-alt yellow";
			   if (msg.user==nickname) bubbleclass="bubble white";
			   //$("#page_chat #chatul").prepend("<li><div class='speech-bubble "+bubbleclass+"'><span class='user'>"+msg.user+"</span><span class='msg'>"+msg.message+"</span></div></li>");
			   $("#page_chat #chatwindow").append("<div class='"+bubbleclass+"'><span class='user'>"+msg.user+"</span><span class='msg'>"+msg.message+"</span></div>");
			   //$("#page_chat #chatwindow").prepend('<p class="triangle-isosceles bubble-silver left"><span class="user">'+msg.user+'</span><span class="msg">'+msg.message+'</span></p>');
			   $("#page_chat #status").empty();
			   
			   var objDiv = document.getElementById("chatwindow");
              objDiv.scrollTop = objDiv.scrollHeight;
			   /*var myDiv = $("#page_chat #chatwindow");
               myDiv.animate({ scrollBottom: myDiv.prop("scrollHeight") - myDiv.height() }, 3000);*/
         });
		
		socket.on('connected', function(msg) {
               colog(msg.user+" joined the chat");
			   $("#page_chat #chatul").append("<div class='bubble'><span class='msg'><i>User <b>"+msg.user+"</b> joined the chat</i></span></div>");
			   $("#page_chat #status").empty();
		...
		socket.on('typing', function(msg) {
               //console.log("received typing from sockserver: "+JSON.stringify(msg));
			   $("#page_chat #status").html(msg.user+" is typing....");
        });
