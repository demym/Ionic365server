(function($) {
   $.jqmCalendar = function(element, options) {
	 ...
	 function tapHoldHandler() {
      ...
      function listItemFormatter($listItem, timeString, summary, event,bg) {
         ...
         if (event[plugin.settings.url]) {
            $('<a></a>').text( text ).attr( 'href', event[plugin.settings.url] ).appendTo($listItem);
			 ...
			 var delbutton="<a data-mini='true' data-role='button' href='#' onclick='deleteEvent(this)'>Delete</a>";
			 $listItem.html( "<a href='#' class='eventlistitem' id='"+event.id+"_"+event.rev+"'>"+event.begin.format('dd/mm/yyyy')+" - "+ text+"</a>" );
			  $(".eventlistitem").on("taphold",function() {
				 ...
				 gConfirm("Do you want to remove this event ?","Event removal",function() {
					  deleteEvent(doc);	   
	  ...
	  $element.bind('refreshall', function(event, events) {
